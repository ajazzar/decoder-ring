...
const polybiusModule = (function () {
  ...
  function polybius(input, encode = true) {
    ...
    let result = "";
    //Higher-order functions are any function that either accepts a function as one of its incoming arguments
  ...
  function encodeFunc(input){
    ...
    for (let i=0; i < input.length; i++){
      ...
      //The forEach() method lets you do this without a for loop, which 
      //lets you think about the items themselves, instead of counting indexes.
      values.forEach((value, index) => {
        if (input[i] === value[0] || input[i] === value[i]){
          result += keys[index]
        }
      }) 
